...
class GridView extends BaseListView
{
     ...
     */
    public $layout = "{select}\n{items}\n<div class='col-sm-6 customSummary'>{summary}</div><div class='col-sm-6 text-right'>{pager}</div>";   
    ...
    public function renderSelect()
    {
        ...
        $optString='';
        $recordscountArr=array('5','10','15','20');
        
        foreach($recordscountArr as $newArr){
            $selected='';
            if($newArr==$pageSize){
                $selected="selected='selected'";
            }
            $optString.='<option '.$selected.' value="'.$newArr.'">'.$newArr.'</option>';
        }
        $dropdown='<form method="get" action="'.$filterUrl.'" name="dropdownForm">
            <select name="noItemSelected" id="noItemSelected" onchange="form.submit()">
